...
- `GET /api/v1/agents/{agent_id}/knowledge/files/info`
- `GET /api/v1/agents/{agent_id}/knowledge/files/content`
- `GET /api/v1/agents/{agent_id}/knowledge/files/download`
- `POST /api/v1/agents/{agent_id}/knowledge/files/upload/background`
- `POST /api/v1/agents/{agent_id}/knowledge/files/base64/background`
- `PUT /api/v1/agents/{agent_id}/knowledge/files/background`
- `GET /api/v1/agents/{agent_id}/knowledge/tasks/pending`
- `GET /api/v1/agents/{agent_id}/knowledge/tasks/{task_id}`
...
If a standalone `<stem>.md` already exists without the matching original upload, the backend preserves that user-authored markdown and allocates the first incremented basename for the new managed pair (for example `brief-1.pdf` + `brief-1.md`). Existing original uploads still follow the current `overwrite=true` / `upsert=true` semantics, and refreshing a managed pair updates both the original file and its generated markdown sibling. Direct upload/replace to a managed generated `<stem>.md` path is rejected with `409 Conflict`, including when the original exists and the markdown sibling is still missing because that path is reserved for the managed pair. Deleting either member of a managed pair removes both files, while standalone user-authored markdown files still upload, replace, and delete normally. Read/info/download operations remain path-based on the specific file you request. `GET /files/info` also surfaces associated background upload task metadata via `task_info.active_task` and `task_info.latest_successful_task`. When the requested path is a managed generated markdown sibling such as `brief.md`, `task_info.association_status` becomes `managed_original` and `task_info.canonical_path` points at the paired original path such as `brief.pdf`. Successful renamed uploads are associated through the final stored `result.path`, so a collision-resolved file like `brief-1.pdf` links back to the succeeded task that actually produced `brief-1.pdf`.
...
- Sono disponibili anche endpoint background per upload knowledge (multipart, base64 e replace) che restituiscono un `task_id`, con `GET /api/v1/agents/{agent_id}/knowledge/tasks/pending` per i task ancora pending e `GET /api/v1/agents/{agent_id}/knowledge/tasks/{task_id}` per lo stato dettagliato. I task pending oltre 30 minuti vengono marcati `expired` e rimossi dalla pending list.
